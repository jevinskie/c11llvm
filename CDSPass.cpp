...
//===----------------------------------------------------------------------===//
//
// This file is a modified version of ThreadSanitizer.cpp, a part of a race
// detector.
//
...
static inline Value *getPosition(Instruction *I, IRBuilder<> &IRB,
                                 bool print = false) {
...
private:
  ...
  FunctionCallee CDSAtomicRMW[AtomicRMWInst::LAST_BINOP + 1]
...
StringRef CDSPass::getPassName() const { return "CDSPass"; }
...
void CDSPass::initializeCallbacks(Module &M) {
  ...
  CDSFuncEntry =
      M.getOrInsertFunction("cds_func_entry", Attr, VoidTy, Int8PtrTy);
  CDSFuncExit = M.getOrInsertFunction("cds_func_exit", Attr, VoidTy, Int8PtrTy);
  ...
  for (unsigned i = 0; i < kNumberOfAccessSizes; i++) {
    ...
    CDSLoad[i] = M.getOrInsertFunction(LoadName, Attr, VoidTy, Int8PtrTy);
    CDSStore[i] = M.getOrInsertFunction(StoreName, Attr, VoidTy, Int8PtrTy);
    CDSVolatileLoad[i] =
        M.getOrInsertFunction(VolatileLoadName, Attr, Ty, PtrTy, Int8PtrTy);
    CDSVolatileStore[i] = M.getOrInsertFunction(VolatileStoreName, Attr, VoidTy,
                                                PtrTy, Ty, Int8PtrTy);
    CDSAtomicInit[i] = M.getOrInsertFunction(AtomicInitName, Attr, VoidTy,
                                             PtrTy, Ty, Int8PtrTy);
    CDSAtomicLoad[i] = M.getOrInsertFunction(AtomicLoadName, Attr, Ty, PtrTy,
                                             OrdTy, Int8PtrTy);
    CDSAtomicStore[i] = M.getOrInsertFunction(AtomicStoreName, Attr, VoidTy,
                                              PtrTy, Ty, OrdTy, Int8PtrTy);
    ...
    for (unsigned op = AtomicRMWInst::FIRST_BINOP;
      ...
      CDSAtomicRMW[op][i] = M.getOrInsertFunction(AtomicRMWName, Attr, Ty,
                                                  PtrTy, Ty, OrdTy, Int8PtrTy);
    }

    // only supportes strong version
    SmallString<32> AtomicCASName_V1("cds_atomic_compare_exchange" +
                                     BitSizeStr + "_v1");
    SmallString<32> AtomicCASName_V2("cds_atomic_compare_exchange" +
                                     BitSizeStr + "_v2");
    CDSAtomicCAS_V1[i] = M.getOrInsertFunction(
        AtomicCASName_V1, Attr, Ty, PtrTy, Ty, Ty, OrdTy, OrdTy, Int8PtrTy);
    CDSAtomicCAS_V2[i] =
        M.getOrInsertFunction(AtomicCASName_V2, Attr, Int1Ty, PtrTy, PtrTy, Ty,
                              OrdTy, OrdTy, Int8PtrTy);
  }

  CDSAtomicThreadFence = M.getOrInsertFunction("cds_atomic_thread_fence", Attr,
                                               VoidTy, OrdTy, Int8PtrTy);

  MemmoveFn = M.getOrInsertFunction("memmove", Attr, Int8PtrTy, Int8PtrTy,
                                    Int8PtrTy, IntPtrTy);
  MemcpyFn = M.getOrInsertFunction("memcpy", Attr, Int8PtrTy, Int8PtrTy,
                                   Int8PtrTy, IntPtrTy);
  MemsetFn = M.getOrInsertFunction("memset", Attr, Int8PtrTy, Int8PtrTy,
                                   Int32Ty, IntPtrTy);
}
...
bool CDSPass::doInitialization(Module &M) {
  ...
  // createSanitizerCtorAndInitFunctions is defined in
  // "llvm/Transforms/Utils/ModuleUtils.h" We do not support it yet
  /*
  ...
  */
  ...
  AtomicFuncNames = {"atomic_init",     "atomic_load",
                     "atomic_store",    "atomic_fetch_",
                     "atomic_exchange", "atomic_compare_exchange_"};

  PartialAtomicFuncNames = {"load", "store", "fetch", "exchange",
                            "compare_exchange_"};
...
bool CDSPass::runOnFunction(Function &F) {
  ...
  // Instrument function entry and exit for functions containing atomics or
  // volatiles
...
bool CDSPass::instrumentLoadOrStore(Instruction *I, const DataLayout &DL) {
  IRBuilder<> IRB(I);
  ...
  if (IsWrite && isVtableAccess(I)) {
    ...
    IRB.CreateCall(TsanVptrUpdate,
                                    {IRB.CreatePointerCast(Addr,
    IRB.getInt8PtrTy()), IRB.CreatePointerCast(StoredValue,
    IRB.getInt8PtrTy())}); NumInstrumentedVtableWrites++;
    */
    return true;
  }
  ...
  if (!IsWrite && isVtableAccess(I)) {
    ...
    IRB.CreateCall(TsanVptrLoad,
                                     IRB.CreatePointerCast(Addr,
    IRB.getInt8PtrTy())); NumInstrumentedVtableReads++;
    */
    return true;
  }
...
bool CDSPass::instrumentAtomic(Instruction *I, const DataLayout &DL) {
  ...
  if (LoadInst *LI = dyn_cast<LoadInst>(I)) {
    ...
    Value *order = ConstantInt::get(OrdTy, atomic_order_index);
    Value *Args[] = {Addr, val, order, position};
    Instruction *funcInst =
        CallInst::Create(CDSAtomicRMW[RMWI->getOperation()][Idx], Args);
    ...
    int atomic_order_index_succ =
        getAtomicOrderIndex(CASI->getSuccessOrdering());
    int atomic_order_index_fail =
        getAtomicOrderIndex(CASI->getFailureOrdering());
...
bool CDSPass::isAtomicCall(Instruction *I) {
  if (auto *CI = dyn_cast<CallInst>(I)) {
    ...
    for (StringRef PartialName : PartialAtomicFuncNames) {
      if (funName.contains(PartialName) && funName.contains("atomic"))
        return true;
    }
  }
...
bool CDSPass::instrumentAtomicCall(CallInst *CI, const DataLayout &DL) {
  ...
  if (funName.contains("atomic_load")) {
    ...
    if (isExplicit)
      ...
      order = ConstantInt::get(OrdTy, (int)AtomicOrderingCABI::seq_cst);
    ...
    return true;
  } else if (funName.contains("atomic") && funName.contains("load")) {
  ...
  if (funName.contains("atomic_store")) {
    ...
    return true;
  } else if (funName.contains("atomic") && funName.contains("store")) {
    // Does this version of call always have an atomic order as an argument?
  ...
  if (funName.contains("atomic_compare_exchange_")) {
    ...
    if (isExplicit) {
      ...
      if (parameters.size() > 4) {
        ...
        order_fail = ConstantInt::get(OrdTy, AtomicCasFailureOrderIndex(index));
      }
    } else {
      order_succ = ConstantInt::get(OrdTy, (int)AtomicOrderingCABI::seq_cst);
      order_fail = ConstantInt::get(OrdTy, (int)AtomicOrderingCABI::seq_cst);
    }
...
int CDSPass::getMemoryAccessFuncIndex(Value *Addr, const DataLayout &DL) {
...
static RegisterStandardPasses
    RegisterMyPass1(PassManagerBuilder::EP_OptimizerLast, registerCDSPass);

/* Enable the pass when opt level is 0 */
static RegisterStandardPasses
    RegisterMyPass2(PassManagerBuilder::EP_EnabledOnOptLevel0, registerCDSPass);
